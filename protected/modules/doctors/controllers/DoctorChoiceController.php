...
class DoctorChoiceController extends Controller
{
    public $menu = array( array( 'link' => 'doctorChoice/create','text'=>'Add new Choice' ));
    ...
    public function actionCreate()
    {
        ...
        {
            $model->attributes = $_POST['DoctorChoice'];

            if( $model->save() )
                $this->redirect( array( 'view', 'id' => $model->id ) );
        }

        $model->order_by = $model->getNextOrder();
        $model->f_status = 1;
    ...
    {
        $this->controls = true;
        $models = DoctorChoice::retrieveAll( null, array() );
    ...
    {
        $model=DoctorChoice::model()->findByPk($id);
        if($model === null)
